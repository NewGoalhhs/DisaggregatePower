...
class IsUsingAppliance(Migration):
    ...
    def insert(self):
        ...
        for appliance in Database.query(DatabaseQuery.SELECT_ALL.format('Appliance')):
            appliance_thresholds[appliance.get('id')] = Database.query(DatabaseQuery.SELECT_APPLIANCE_THRESHOLD.format(appliance.get('id')))[0].get('appliance_power')
        ...
        for building_id in building_ids:
            ...
            for power_usage in power_usage_appliance:
                if power_usage.get('appliance_power') > appliance_thresholds[power_usage.get('Appliance_id')]:
                    self.add_sql(Query.INSERT_IS_USING_APPLIANCE.format(power_usage.get('PowerUsage_id'), power_usage.get('Appliance_id')))
                lb.update(1)
            lb.finish()
